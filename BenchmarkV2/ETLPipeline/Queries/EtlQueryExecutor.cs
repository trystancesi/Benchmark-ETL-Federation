...
namespace ETLPipeline.Queries;

/// <summary>
/// Exécute les scénarios T1 à T4 sur la base centralisée ETL.
/// Toutes les tables sont co-localisées — pas de jointure distribuée.
/// </summary>
public class EtlQueryExecutor
{
    private readonly string _centralizedConnectionString;

    public EtlQueryExecutor(string centralizedConnectionString)
    {
        _centralizedConnectionString = centralizedConnectionString;
    }

    /// <summary>
    /// T1 – Liste des SN (SELECT simple avec ORDER BY).
    /// </summary>
    ...
    /// <summary>
    /// T2 – Mesures par type de pièce (4 jointures).
    /// </summary>
    public async Task<long> ExecuteT2_GetMesuresByPNAsync(string sn)
    {
        const string sql = @"
            ...
            FROM Piece p
            JOIN Lien_type_mesure_type_piece LTMC
                ON LTMC.ID_PN = p.ID_PN
                AND LTMC.Is_Active = 1
            JOIN Type_mesure CTM
    ...
    /// <summary>
    /// T3 – Historique complet des mesures d'une pièce (CTE + fenêtrage + LAG).
    /// </summary>
        ...
        const string sql = @"
            ...
            MesuresActuelles AS (
                SELECT
                    M.ID_Mesure,
                    M.ID_SN,
                    M.ID_Lien_Type_Mesure_Catalogue,
                    M.Valeur,
                    M.Commentaire,
                    M.Insert_Date,
                    M.ID_Operateur,
                    M.Is_Active,
                    M.ID_Mesure AS CurrentMesureID
                FROM Mesures AS M
            ...
            MesuresHist AS (
                SELECT
                    MPrev.ID_Mesure,
                    MPrev.ID_SN,
                    MPrev.ID_Lien_Type_Mesure_Catalogue,
                    MPrev.Valeur,
                    MPrev.Commentaire,
                    MPrev.Insert_Date,
                    MPrev.ID_Operateur,
                    MPrev.Is_Active,
                    MA.ID_Mesure AS CurrentMesureID
                FROM Historique H
            ...
            SELECT
                ...
                LTMC.Warning_Level,
                CASE
                    WHEN d.ID_Derogation IS NULL THEN -1
                    ELSE d.Accepted
                END AS Derog,
                O.Valeur AS Result,
                O.Commentaire,
                O.Insert_Date,
                CASE
                    WHEN O.Prev_ID IS NULL THEN O.ID_Operateur
                    ELSE Hprev.ID_Operateur
                END AS ID_Operateur,
                O.Is_Active
            FROM SN_CTE S
            INNER JOIN Lien_type_mesure_type_piece LTMC
                ON LTMC.ID_PN = S.ID_PN AND LTMC.Is_Active = 1
            INNER JOIN Type_mesure CTM
            ...
            LEFT JOIN Derogations d
                ON d.ID_Mesure  = O.ID_Mesure
               AND d.Is_Active  = 1
            ORDER BY LTMC.Numero_Etape, LTMC.Ordre_Mesure,
                CASE
                    WHEN O.Prev_ID IS NULL THEN O.Insert_Date
                    ELSE Hprev.Date_Insertion
                END;";
    ...
    /// <summary>
    /// T4 – Insertion d'une mesure avec OUTPUT INSERTED.ID_Mesure.
    /// </summary>
        ...
        decimal valeur, int idSN, int idLien, int idOperateur, string? commentaire = null)
    {
        ...
        await using var conn = new SqlConnection(_centralizedConnectionString);
